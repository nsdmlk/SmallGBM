...
static Scratch *scratch_create(int cap) {
    ...
    s->bin_pool  = malloc((size_t)NBINS * sizeof(int));

    if (!s->residuals || !s->scratch || !s->left_idx || !s->right_idx ||
        !s->hist_cnt || !s->hist_sum || !s->cum_cnt || !s->cum_sum || !s->bin_pool) {
...
static BinnedData *binned_create(const double *X, int n, int m) {
    ...
    for (int f = 0; f < m; f++) {
        ...
        /* БИНАРИЗАЦИЯ: lo в [0, NBINS-1], чтобы не переполнять uint8_t */
        for (int i = 0; i < n; i++) {
...
static BestSplit best_split(const BinnedData *bd, const double *y,
    ...
    /* colsample_bytree: выбрать подмножество фич */
    int n_cols = m;
    int *feat_pool = s->bin_pool;  /* переиспользуем как pool для фич */
    for (int j = 0; j < m; j++) feat_pool[j] = j;
    if (t->colsample_bytree < 1.0 && m > 1) {
        n_cols = (int)(m * t->colsample_bytree);
        if (n_cols < 1) n_cols = 1;
        for (int i = m - 1; i > 0; i--) {
            int j = rand() % (i + 1);
            int tmp = feat_pool[i]; feat_pool[i] = feat_pool[j]; feat_pool[j] = tmp;
        }
    } else {
        n_cols = m;
    }
    ...
    for (int fi = 0; fi < n_cols; fi++) {
        ...
        /* валидный диапазон порогов: b в [b_lo, b_hi] */
        ...
        int n_try = (n_splits < range) ? n_splits : range;

        for (int sp = 0; sp < n_try; sp++) {
...
static Node *make_leaf(const double *y, const int *idx, int n,
    ...
    RobustWeight w = robust_weight(s->residuals, n, parent_mu,
                                   sigma_prior, s->scratch);
...
static Node *build_tree(const Tree *t, const BinnedData *bd, const double *y,
    ...
    if (!stop) {
        int same = 1;
        for (int i = 1; i < n; i++) {
            if (y[idx[i]] != y[idx[0]]) { same = 0; break; }
        }
        if (same) stop = 1;
    }
    ...
    for (int i = 0; i < n; i++) {
        int k = idx[i];
        if (bd->Xb[(size_t)k * m + split.best_feature] <= split.best_bin)
            li[nl++] = k;
        else
            ri[nr++] = k;
    }

    /* защита: если split вырожденный — лист */
    if (nl < t->min_samples_leaf || nr < t->min_samples_leaf)
        return make_leaf(y, idx, n, parent_mu, t->sigma_prior, s);
    ...
    node->type        = NODE;
    node->feature     = split.best_feature;
    node->threshold   = split.best_threshold;
    node->weight      = node_w.mu;
    node->uncertainty = node_w.std;
    node->n_samples   = n;
...
Tree *tree_create(int max_depth, int min_samples_leaf, double sigma_prior,
    ...
    t->min_samples_leaf = min_samples_leaf;
    t->sigma_prior      = sigma_prior;
    t->n_splits         = n_splits;
    t->colsample_bytree = colsample_bytree;
    t->tree             = NULL;
    return t;
}
